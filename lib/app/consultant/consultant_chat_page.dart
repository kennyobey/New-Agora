...
import 'package:agora_care/app/consultant/widget/full_photo_page.dart';
...
class ConsultChatPageState extends State<ConsultChatPage> {
  ...
  // late AuthControllers _authController;
  ...
  Future uploadFile() async {
    ...
    UploadTask uploadTask = _chatProvider.uploadFile(imageFile!, fileName);
    try {
  ...
  Widget buildSticker() {
    return Expanded(
      child: Container(
        decoration: BoxDecoration(
            border: Border(
                top: BorderSide(color: AppColor().flareColor, width: 0.5)),
            color: Colors.white),
    ...
    return Flexible(
      child: groupChatId.isNotEmpty
          ? StreamBuilder<QuerySnapshot>(
              stream: _chatProvider.getChatStream(groupChatId, _limit),
